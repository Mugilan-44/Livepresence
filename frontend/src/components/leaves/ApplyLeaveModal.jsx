...
export default function ApplyLeaveModal({ onClose, onSubmitLeave, activeUser, policy }) {
  const leaveTypes = useMemo(() => {
    const allocation = policy?.yearly_allocation || {};
    return [
      { value: 'Casual Leave', label: 'Casual Leave', enabled: Number(allocation.casual ?? 12) > 0 },
      { value: 'Sick Leave', label: 'Sick Leave', enabled: Number(allocation.sick ?? 10) > 0 },
      { value: 'Earned Leave', label: 'Earned Leave', enabled: Number(allocation.earned ?? 15) > 0 },
      { value: 'Compensatory Off', label: 'Comp Off', enabled: Number(allocation.comp_off ?? 2) > 0 }
  ...
  const handleSubmit = async (e) => {
    ...
    const days = Math.floor((Date.parse(`${formData.endDate}T00:00:00Z`) - Date.parse(`${formData.startDate}T00:00:00Z`)) / 86400000) + 1;
    setLoading(true);
    setError('');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl max-w-5xl w-full max-h-[94vh] overflow-y-auto p-7 sm:p-9 shadow-2xl space-y-7">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          ...
          <div>
            ...
            <textarea
              ...
              className="min-h-80 w-full resize-y p-4 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white focus:outline-none focus:border-cyan-500"
            />
          </div>
