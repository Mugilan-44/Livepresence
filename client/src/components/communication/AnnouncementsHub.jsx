...
import { Bell, Calendar, PartyPopper, Award, FileText, Megaphone, Plus } from 'lucide-react';
import CreateAnnouncementModal from './CreateAnnouncementModal';

export default function AnnouncementsHub({ announcements, events, onPublishAnnouncement, activeUser }) {
            ...
            {announcements?.map((item) => (
              <div key={item.id} className="bg-slate-50/70 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-xl p-4 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] px-2.5 py-0.5 rounded-full bg-purple-500/10 text-purple-600 font-bold border border-purple-500/30">
                    {item.category}
                  </span>
                </div>
          ...
          <div className="space-y-3">
            {events?.map((ev) => (
              <div key={ev.id} className="flex items-center justify-between text-xs p-3 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                ...
                <span className="font-mono text-slate-600 dark:text-slate-300 font-semibold">{ev.date}</span>
              </div>
            ))}
          </div>
        </div>
