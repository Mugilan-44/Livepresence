...
export default function SmtpSettingsModal({ isOpen, onClose, activeUser }) {
  ...
  const fetchSmtpData = async () => {
    try {
      const [cfgRes, logRes] = await Promise.all([
        safeFetchJson('/api/smtp/config', { headers: { 'x-user-role': activeUser?.role || 'SUPER_ADMIN' } }).catch(e => ({})),
        safeFetchJson('/api/smtp/logs', { headers: { 'x-user-role': activeUser?.role || 'SUPER_ADMIN' } }).catch(e => ([]))
      ]);
  ...
  const handleVerifyConnection = async () => {
    try {
      setVerifying(true);
      setSaveMsg(null);
      const data = await safeFetchJson('/api/smtp/verify', {
        method: 'POST',
  ...
  const handleSaveConfig = async (e) => {
    ...
    try {
      ...
      const data = await safeFetchJson('/api/smtp/config', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...smtpConfig,
  ...
  const handleSendTestEmail = async () => {
    ...
    try {
      ...
      const data = await safeFetchJson('/api/smtp/test-email', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
