...
export default function LeaveManagementPage({ activeUser, users = [], onRefreshLeaves }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Top Bar Header with Role-Switching Capability for Admins */}
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-slate-900 dark:text-white">
              {viewMode === 'admin' ? 'Enterprise Leave Governance' : 'Leave Portal'}
            </h2>
          </div>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            {viewMode === 'admin'
              ? 'Manage company leave policies, approvals, employee balances, and official holidays.'
              : 'Track your yearly leave balances, apply for time off, view requests, and check upcoming holidays.'}
          </p>
        </div>

        {/* Toggle Mode for Admins */}
        {isAdmin && (
