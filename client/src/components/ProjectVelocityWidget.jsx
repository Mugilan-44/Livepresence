...
export default function ProjectVelocityWidget({
  ...
  const canScopeProject = ['SUPER_ADMIN', 'HR', 'MANAGER', 'FULL_TIME'].includes(activeUser?.role);
  ...
  const scopedProjects = useMemo(() => {
    ...
    return projects.filter(p => {
      ...
      const isLead = p.lead_id === activeUser?.id || p.lead_name === activeUser?.name;
  ...
  const filteredProjects = useMemo(() => {
    if (!searchQuery.trim()) return scopedProjects;
  ...
  const notStartedCount = scopedTasks.filter(t => t.status === 'To Do' || t.status === 'Not Started').length;
  const ongoingTasksCount = scopedTasks.filter(t => t.status === 'In Progress' || t.status === 'Code Review' || t.status === 'Testing').length;
  const completedTasksCount = scopedTasks.filter(t => t.status === 'Completed' || t.status === 'Done').length;

  const dynamicStatuses = customizations?.task_statuses || [
    { id: 'todo', label: 'To Do' },
    { id: 'in_progress', label: 'In Progress' },
    { id: 'code_review', label: 'Code Review' },
    { id: 'completed', label: 'Completed' }
  ];
  const stages = dynamicStatuses.map(s => s.label);
  ...
  return (
    <div className="space-y-6 select-none">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-[#0d1322] border border-slate-200 dark:border-slate-800 border-l-4 border-l-blue-500 rounded-2xl p-4 shadow-sm flex items-center justify-between transition hover:shadow-md">
        ...
        <div className="bg-white dark:bg-[#0d1322] border border-slate-200 dark:border-slate-800 border-l-4 border-l-rose-500 rounded-2xl p-4 shadow-sm flex items-center justify-between transition hover:shadow-md">
          <div className="flex items-center gap-3">
        ...
        <div className="bg-white dark:bg-[#0d1322] border border-slate-200 dark:border-slate-800 border-l-4 border-l-blue-400 rounded-2xl p-4 shadow-sm flex items-center justify-between transition hover:shadow-md">
          <div className="flex items-center gap-3">
        ...
        <div className="bg-white dark:bg-[#0d1322] border border-slate-200 dark:border-slate-800 border-l-4 border-l-emerald-500 rounded-2xl p-4 shadow-sm flex items-center justify-between transition hover:shadow-md">
          <div className="flex items-center gap-3">
      ...
      <div className="bg-white dark:bg-[#0d1322] border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {['SUPER_ADMIN', 'HR', 'MANAGER'].includes(activeUser?.role) && (
              <button
                ...
                <div className="p-5 border-t border-slate-200 dark:border-slate-800 space-y-4 bg-white dark:bg-[#0d1322]">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 pt-2">
                    {stages.map((stage) => {
                      ...
                      return (
                        <div key={stage} className="bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/80 rounded-xl p-3 flex flex-col min-h-[200px]">
                          <div className="flex items-center justify-between pb-2 mb-2 border-b border-slate-200 dark:border-slate-800">
                            <span className="text-xs font-bold text-slate-900 dark:text-white uppercase tracking-wider flex items-center gap-1.5">
                              <span className={`w-2 h-2 rounded-full ${
                                stage === 'Completed' || stage === 'Done' ? 'bg-emerald-500' :
                                stage === 'In Progress' ? 'bg-blue-500' :
                                stage === 'Code Review' ? 'bg-purple-500' :
                                stage === 'Testing / QA' ? 'bg-amber-500' :
                                stage === 'Blocked / Escalated' ? 'bg-rose-500' : 'bg-slate-400'
                              }`} />
                              {stage}
                            </span>
                          ...
                          <div className="space-y-2.5 flex-1">
                            {stageTasks.map((t) => (
                              <div key={t.id} className="bg-white dark:bg-slate-800/90 border border-slate-200 dark:border-slate-700/60 hover:border-cyan-500/50 rounded-xl p-3 transition shadow-2xs space-y-2">
